<div align="center">

<img src="./assets/profile-banner.gif" width="100%" alt="Monochrome cyberpunk profile banner">

# Alisson Machado

**Cybersecurity student focused on AppSec, OffSec, web security and automation.**

`break to understand | build to harden | repeat`

</div>

---

## About me

I'm a **Systems Analysis and Development** student building a career in cybersecurity.

My current focus is understanding how software, networks and web applications fail, so I can learn how to build systems that are harder to break. I am especially interested in offensive security, application security, security tooling, bug bounty workflows and automation.

I also have hands-on experience with **IT support / help desk**, troubleshooting, networks, Windows, Linux and technical service, which helps me approach security with a practical, system-level mindset.

---

## Stack & Tools

<div align="left">

![Python](https://img.shields.io/badge/Python-111111?style=for-the-badge&logo=python&logoColor=3776AB)
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![C++](https://img.shields.io/badge/C++-111111?style=for-the-badge&logo=cplusplus&logoColor=00599C)
![Go](https://img.shields.io/badge/Go-111111?style=for-the-badge&logo=go&logoColor=00ADD8)
![SQL](https://img.shields.io/badge/SQL-111111?style=for-the-badge&logo=postgresql&logoColor=4169E1)
![Linux](https://img.shields.io/badge/Linux-111111?style=for-the-badge&logo=linux&logoColor=FCC624)
![Git](https://img.shields.io/badge/Git-111111?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=FFFFFF)
![Burp Suite](https://img.shields.io/badge/Burp%20Suite-111111?style=for-the-badge&logo=burpsuite&logoColor=FF6633)
![Automation](https://img.shields.io/badge/Automation-111111?style=for-the-badge&logo=gnubash&logoColor=4EAA25) 
![Binary](https://img.shields.io/badge/Binary-111111?style=for-the-badge&logo=opsgenie&logoColor=FFFFFF)        
![Spidering](https://img.shields.io/badge/Spidering-111111?style=for-the-badge)

</div>

---

## Currently learning

- Networking fundamentals and Linux internals
- Cloud and security fundamentals
- Secure programming and vulnerability analysis
- Binary fundamentals and low-level data representation
- Web application security testing
- Recon automation and scripting with Python
- Performance, testing and clean tooling practices

---

<div align="center">
  <img src="./assets/spider-banner.jpeg" width="100%" alt="Spider crawler banner">
</div>

---

## Main interests

- Application Security
- Offensive Security
- Web Security
- Bug Bounty
- Security Tooling
- Exploit Research
- Recon and automation
- AI applied to cybersecurity
- Spider-style crawlers, mapping and discovery tools

I also have a fascination with spiders, both the real ones and the "spider" tools that crawl applications, map attack surfaces and reveal hidden paths.

---

## Featured projects

### pycracklab

A Python lab for controlled brute-force experiments, built for learning about performance, testing, limits, safe execution and good engineering practices.

### Security automation ideas

Scripts and tools focused on recon, vulnerability analysis, workflow automation and practical security learning.

### Systems and monitoring projects

Ideas involving digital tags, inventory/logistics control and Arduino-based monitoring.

---

## Mindset

<p align="center">
  <img src="./assets/eye-banner.gif" width="100%" alt="Monochrome eye banner">
</p>

I like learning how things break because that is where the deeper lessons usually are.

My approach is simple: stay disciplined, study the fundamentals, build consistently and keep improving one layer at a time.

---

## Connect with me

- Email: `alissonmc.dev@gmail.com`
