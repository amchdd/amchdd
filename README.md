<div align="center">

<img src="./assets/profile-banner.gif" width="100%" alt="Banner de perfil cyberpunk monocromático">

# Alisson Machado

**Estudante de cibersegurança com foco em AppSec, OffSec, segurança web e automação.**

`quebrar para entender | construir para fortalecer | repetir`

</div>

---

<img align="left" src="./assets/decor/brain.png" width="190" alt="Ilustração monocromática de um cérebro">

## Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas** e estou construindo minha carreira em cibersegurança.

Meu foco atual é entender como softwares, redes e aplicações web falham, para aprender a desenvolver sistemas mais difíceis de comprometer. Tenho interesse especial em segurança ofensiva, segurança de aplicações, desenvolvimento de ferramentas de segurança, fluxos de bug bounty e automação.

Também tenho experiência prática com **suporte de TI e help desk**, resolução de problemas, redes, Windows, Linux e atendimento técnico. Essa experiência me ajuda a abordar segurança com uma visão prática e sistêmica.

<br clear="both">

---

<img align="right" src="./assets/decor/eye.png" width="190" alt="Ilustração monocromática de um olho">

## Stack e ferramentas

<div align="left">

![Python](https://img.shields.io/badge/Python-111111?style=for-the-badge&logo=python&logoColor=3776AB)
![JavaScript](https://img.shields.io/badge/JavaScript-111111?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Rust](https://img.shields.io/badge/Rust-111111?style=for-the-badge&logo=rust&logoColor=CE412B)
![Go](https://img.shields.io/badge/Go-111111?style=for-the-badge&logo=go&logoColor=00ADD8)
![SQL](https://img.shields.io/badge/SQL-111111?style=for-the-badge&logo=postgresql&logoColor=4169E1)
![Linux](https://img.shields.io/badge/Linux-111111?style=for-the-badge&logo=linux&logoColor=FCC624)
![Git](https://img.shields.io/badge/Git-111111?style=for-the-badge&logo=git&logoColor=F05032)
![GitHub](https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=FFFFFF)
![Burp Suite](https://img.shields.io/badge/Burp%20Suite-111111?style=for-the-badge&logo=burpsuite&logoColor=FF6633)
![Automação](https://img.shields.io/badge/Automa%C3%A7%C3%A3o-111111?style=for-the-badge&logo=gnubash&logoColor=4EAA25)
![Binários](https://img.shields.io/badge/Bin%C3%A1rios-111111?style=for-the-badge&logo=opsgenie&logoColor=FFFFFF)
![TypeScript](https://img.shields.io/badge/TypeScript-111111?style=for-the-badge&logo=typescript&logoColor=3178C6)
![n8n](https://img.shields.io/badge/n8n-111111?style=for-the-badge&logo=n8n&logoColor=EA4B71)

</div>

<br clear="both">

---

<img align="left" src="./assets/decor/statue.png" width="180" alt="Busto clássico monocromático">

## Atualmente estudando

- Fundamentos de redes e funcionamento interno do Linux
- Fundamentos de computação em nuvem e segurança
- Programação segura e análise de vulnerabilidades
- Fundamentos de binários e representação de dados em baixo nível
- Testes de segurança em aplicações web
- Automação de reconhecimento e scripts com Python
- Desempenho, testes e boas práticas no desenvolvimento de ferramentas

<br clear="both">

---

<div align="center">
  <img src="./assets/spider-banner.jpeg" width="100%" alt="Banner de crawler em formato de aranha">
</div>

---

<img align="right" src="./assets/decor/mask.png" width="180" alt="Máscara teatral monocromática">

## Principais interesses

- Segurança de Aplicações
- Segurança Ofensiva
- Segurança Web
- Bug Bounty
- Desenvolvimento de Ferramentas de Segurança
- Pesquisa de Exploits
- Reconhecimento e automação
- Inteligência artificial aplicada à cibersegurança
- Crawlers, mapeamento e ferramentas de descoberta inspiradas em aranhas

Também tenho fascínio por aranhas, tanto pelas reais quanto pelas ferramentas do tipo "spider", que percorrem aplicações, mapeiam superfícies de ataque e revelam caminhos ocultos.

<br clear="both">

---

## Projetos em destaque

### [Subdomainabber](https://github.com/amchdd/subdomainabber)

Scanner de segurança desenvolvido em Go para investigar possíveis casos de takeover de subdomínio e configurações incorretas de DNS, correlacionando DNS, HTTP, TLS, provedores e evidências reproduzíveis.

### Ideias de automação de segurança

Scripts e ferramentas voltados para reconhecimento, análise de vulnerabilidades, automação de fluxos de trabalho e aprendizado prático em segurança.

### Projetos de sistemas e monitoramento

Ideias envolvendo etiquetas digitais, controle de inventário e logística, além de monitoramento com Arduino.

---

## Mentalidade

<p align="center">
  <img src="./assets/eye-banner.gif" width="100%" alt="Banner monocromático de um olho">
</p>

Gosto de aprender como as coisas quebram, porque geralmente é aí que estão as lições mais profundas.

Minha abordagem é simples: manter a disciplina, estudar os fundamentos, construir com consistência e continuar evoluindo uma camada de cada vez.

---

## Entre em contato

- E-mail: [alissonmc.dev@gmail.com](mailto:alissonmc.dev@gmail.com)
- LinkedIn: [www.linkedin.com/in/amchdd](https://www.linkedin.com/in/amchdd)
